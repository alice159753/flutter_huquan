...
import 'package:flutterhuquan/pages/login/login_router.dart';
import 'package:flutterhuquan/provider/theme_provider.dart';
import 'package:flutterhuquan/routers/fluro_navigator.dart';
import 'package:flutterhuquan/util/image_utils.dart';
import 'package:flutterhuquan/util/log_utils.dart';
...
class _SplashPageState extends State<SplashPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (SpUtil.getBool(Constants.keyGuide, defValue: true)) {
  ...
  void _initSplash() {
    _subscription = Observable.just(1).delay(Duration(milliseconds: 1500)).listen((_) {
      if (SpUtil.getBool(Constants.keyGuide, defValue: true)) {
        SpUtil.putBool(Constants.keyGuide, false);
        _initGuide();
      } else {
        _goLogin();
      }
